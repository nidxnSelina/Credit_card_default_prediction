...
Tuning the hyperparater max_depth using Grid search
```{r}
max_depths = c(2,4,6,8)
...
best_ER
best_maxDepth # same with default
```
...
library(fastDummies)
categorical_preds <- c("SEX","EDUCATION","MARRIAGE","PAY_SEPT","PAY_AUG", "PAY_JUL", "PAY_JUN", "PAY_MAY", "PAY_APR")
ccard4<- dummy_cols(ccard2, select_columns = categorical_preds, remove_first_dummy = TRUE)

#Delete these redundant variables after one hot coding
ccard4$SEX <- NULL
ccard4$EDUCATION <- NULL
ccard4$MARRIAGE <- NULL
ccard4$PAY_SEPT <- NULL
ccard4$PAY_AUG <- NULL
ccard4$PAY_JUL <- NULL
ccard4$PAY_JUN <- NULL
ccard4$PAY_MAY <- NULL
ccard4$PAY_APR <- NULL
#Turn Y variable into numerical format
ccard4$IsDefaulter <- as.numeric(ccard4$IsDefaulter)-1 
##Neural network
library(neuralnet)
names(ccard4) <- gsub("-", "_", names(ccard4))  # The "-" in column names will cause additional problems in model fitting, thus we need to rename them.

train = sample(nrow(ccard4), nrow(ccard4)*0.7, replace = FALSE)
ccard.train = ccard4[train,]
ccard.test = ccard4[-train,]

nn.ccard = neuralnet(IsDefaulter~.,data=ccard.train ,linear.output=F, hidden=c(3))

nn.pred = predict(nn.ccard,newdata= ccard.test)

nn.pred.class <- ifelse(nn.pred > 0.5, 1, 0)   #The Original prediction output is probability rather than categories, therefore we need to turn nn.pred into classes  
accuracy <- mean(nn.pred.class == ccard.test$IsDefaulter)
neuralER = 1 - accuracy
```
...
While most models yields similar precision in predicting the case of default, Logistic regression, LDA, and tree yield the best performance
- The relationship between our listed predictors and target are simple and can be easily interpreted
- Decision boundary between defaulter and non-defaulters is close to linear

Significant predictors: Repayment amount and status of September, Average amount of bill statement, Age, Limit credit
- More recent repayment information exhibit greater predictive efficacy. Information collected from consecutive months are highly correlated.
- Clients who didn’t pay duly usually paid 2 months late; very few of them paid only one month late
